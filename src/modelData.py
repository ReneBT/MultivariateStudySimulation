...
#              (np.sum(self.p_train)-1)*(Q1-self.AUCROC_cal**2)
        ...
        C = np.bincount(((self.p_train)*2 + (self.pathology_Pred_cal[:,0]>0.5)).ravel()) 
        ...
        C = np.bincount(((self.p_test)*2 + (self.pathology_Pred_val[:,0]>0.5)).ravel()) 
    ...
    def project_models( self , simObj , simSpec  ):
        YM_plsR2_val = self.YM_plsModel.score(simSpec.T-self.meanSpec, 
                                              simObj.simDat['Youngs Modulus'])
        pathology_Pred = self.pathology_plsModel.predict(simSpec.T-self.meanSpec).reshape(-1)        
        AUCROC  = roc_auc_score( simObj.pathology, pathology_Pred )
        C = np.bincount(((simObj.pathology)*2 + (pathology_Pred>0.5)).ravel()) 
