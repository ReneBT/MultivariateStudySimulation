...
### figures of base pure spectra    
 ...
 ### generate observation spectra   
    def specSample( self , simSam , samErr , mnCnts ):
        ...
        self.sample_variation = 10**(stats.norm.rvs( 0 , 0.005, size=( simSam.shape ) ) * samErr * (simSam)**0.5)
        ...
        self.signalInt = stats.norm.rvs( mnCnts , mnCnts**0.5*samErr , 
                                        size=( simSam.shape[0] ) ) # variability of focus affected by spot size which is encoded in samErr
        ...
        for p in range(len(params)):
            ...
            if params[p]+' Scar' in self.constituents:
                ...
                self.samSpec = ( self.samSpec + 
                           np.outer(self.constituentSpec[params[p]+' Scar'],simSam[params[p]]*simSam['Scar']  ))
            ...
            if params[p]+' HPO4_2H2O' in self.constituents:
                ...
                self.samSpec = ( self.samSpec + 
                           np.outer(self.constituentSpec[params[p]+' HPO4_2H2O'],simSam[params[p]]*simSam['HPO4_2H2O']  ))
                if np.sum(np.isnan(self.samSpec))>0:
                    print(str(p) + ': ' + str(np.sum(np.isnan(self.samSpec))))
                    return
            ...
            if params[p]+' CML' in self.constituents:
                ...
                self.samSpec = ( self.samSpec + 
                           np.outer(self.constituentSpec[params[p]+' CML'],simSam[params[p]]*totAGEacid  ) )
                if np.sum(np.isnan(self.samSpec))>0:
                    print(str(p) + ': ' + str(np.sum(np.isnan(self.samSpec))))
                    return
            ...
            if params[p]+' GH1' in self.constituents:
                ...
                self.samSpec = ( self.samSpec + 
                           np.outer(self.constituentSpec[params[p]+' GH1'],simSam[params[p]]*simSam['GH1']  ) )
                if np.sum(np.isnan(self.samSpec))>0:
                    print(str(p) + ': ' + str(np.sum(np.isnan(self.samSpec))))
                    return
            ...
            if params[p]+' CEP' in self.constituents:
                ...
                self.samSpec = ( self.samSpec + 
                           np.outer(self.constituentSpec[params[p]+' CEP'],simSam[params[p]]*simSam['CEP']  ) )
                if np.sum(np.isnan(self.samSpec))>0:
                    print(str(p) + ': ' + str(np.sum(np.isnan(self.samSpec))))
                    return
            ...
            if params[p]+' Pentosidine' in self.constituents:
                ...
                self.samSpec = ( self.samSpec + 
                           np.outer(self.constituentSpec[params[p]+' Pentosidine'],simSam[params[p]]*simSam['Pentosidine']  ) )
                if np.sum(np.isnan(self.samSpec))>0:
                    print(str(p) + ': ' + str(np.sum(np.isnan(self.samSpec))))
                    return
            H[H>1] = 1
            ...
            if params[p]+' H' in self.constituents:
                self.samSpec = ( self.samSpec + 
                           np.outer(self.constituentSpec[params[p]+' H'],H*simSam[params[p]]  ) )
                if np.sum(np.isnan(self.samSpec))>0:
                    print(str(p) + ': ' + str(np.sum(np.isnan(self.samSpec))))
                    return
            if params[p]+' D' in self.constituents:
                self.samSpec = ( self.samSpec + 
                           np.outer(self.constituentSpec[params[p]+' D'],D*simSam[params[p]]  ) )
                if np.sum(np.isnan(self.samSpec))>0:
                    print(str(p) + ': ' + str(np.sum(np.isnan(self.samSpec))))
                    return
            if params[p] in self.constituents: #if there are no deviations it will be unappended
                self.samSpec = ( self.samSpec + 
                           np.outer(self.constituentSpec[params[p]],simSam[params[p]]  ) )
                if np.sum(np.isnan(self.samSpec))>0:
                    print(str(p) + ': ' + str(np.sum(np.isnan(self.samSpec))))
                    return
        ...
        for iP in self.proteins:
            for iAA in range(self.AA_comp.shape[1]):
                ...
                if cAA + ' D' in self.constituents:
                    ...
                    if np.sum(np.isnan(self.samSpec))>0:
                        print(str(iP) + ' ' + str(iAA) +  ': ' + str(np.sum(np.isnan(self.samSpec))))
                        return
                if cAA + ' H' in self.constituents:
                    ...
                    if np.sum(np.isnan(self.samSpec))>0:
                        print(str(iP) + ' ' + str(iAA) +  ': ' + str(np.sum(np.isnan(self.samSpec))))
                        return
        self.samSpec = self.samSpec/np.mean(self.samSpec,axis=0)*self.signalInt

        if np.sum(np.isnan(self.samSpec))>0:
            print('Intensity: ' + str(np.sum(np.isnan(self.samSpec))))
            print(params)
        self.samSpec[np.isnan(self.samSpec)] = 0
